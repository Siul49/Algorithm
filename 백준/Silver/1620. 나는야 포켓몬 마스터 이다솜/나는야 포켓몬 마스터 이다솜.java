...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        HashMap<String, Integer> sitePassword = new HashMap<>(n);
        HashMap<Integer, String> passwordSite = new HashMap<>(n);

        for (int i = 1; i <= n; i++) {
            String poketmon = br.readLine();
            sitePassword.put(poketmon, i);
            passwordSite.put(i, poketmon);
        }

        for (int i = 1; i <= k; i++) {
            String s = br.readLine();
            if (sitePassword.containsKey(s)) {
                bw.write(sitePassword.get(s) + "\n");
            }
            else {
                bw.write(passwordSite.get(Integer.parseInt(s))+ "\n");
            }
        }
        bw.flush();
    }
